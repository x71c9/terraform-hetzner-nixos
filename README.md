...
module "nixos_server" {
  source = "git::https://github.com/x71c9/terraform-hetzner-nixos.git?ref=v1.0.2"
...
| `ssh_key_ids` | `list(string)` | `[]` | List of existing SSH key IDs in Hetzner Cloud. If empty, a new SSH key will be created from `ssh_public_key_path`. This allows using one or more existing SSH keys across multiple servers |
...
module "web_server" {
  ...
  ssh_key_ids         = [hcloud_ssh_key.shared.id]
  ssh_public_key_path = "~/.ssh/id_rsa.pub"
}
