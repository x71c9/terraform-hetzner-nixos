...
resource "hcloud_server" "server" {
  ...
  user_data = md5(join("", [for f in fileset(var.flake_path, "**/*.nix") : f != "configuration.nix" ? filemd5("${var.flake_path}/${f}") : ""]))  
}
...
resource "null_resource" "nixos_deployment" {
  ...
  triggers = {
    nix_config_hash = md5(join("", [for f in fileset(var.flake_path, "**/*.nix") : f != "configuration.nix" ? filemd5("${var.flake_path}/${f}") : ""]))
    server_id       = hcloud_server.server.id
    ssh_public_key  = trimspace(file(var.ssh_public_key_path))
    hostname        = var.host_name
  }
